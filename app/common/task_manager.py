...
from app.common.constants import (
    BROWSER_TASKS_STREAM,
    TASK_RESULTS_KEY_SUFFIX,
    REDIS_RESULT_EXPIRATION_SECONDS,
    REDIS_COOKIES_KEY_FMT,
)
...
class TaskManager:
        ...
        try:
            ...
            if not await self.ensure_redis_connection():
                return BrowserTaskResponse(
                    task_id=task_id,
                    task_status=BrowserTaskStatus.FAILED,
                    task_response="Failed to connect to Redis",
                )
            ...
            initial_result = BrowserTaskResponse(
                task_id=task_id, task_status=BrowserTaskStatus.WAITING
            )
            entry = TaskEntry(
                task_id=task_id,
                request=task_data,
            ...
            # Add to task stream
            log_ctx.info(
                "Adding task to Redis stream", task_id=task_id, task_data=task_data
            )
            ...
            return BrowserTaskResponse(
                task_id=task_id,
                task_status=BrowserTaskStatus.FAILED,
                task_response=f"Error submitting task: {e}",
            )
    ...
    async def get_task_result(self, task_id) -> BrowserTaskResponse:
        ...
        try:
            ...
            return BrowserTaskResponse(
                task_id=task_id,
                task_status=BrowserTaskStatus.FAILED,
                task_response="Result not found",
            )
            ...
            return BrowserTaskResponse(
                task_id=task_id,
                task_status=BrowserTaskStatus.FAILED,
                task_response=f"Error getting result: {str(e)}",
            )
    ...
    async def create_consumer_group(self, task_queue_name: str) -> bool:
        ...
        try:
            ...
            log.exception(
                "Failed to create consumer group", error=str(e), exc_info=True
            )
            return False

    async def read_next_task(
        self, consumer_name: str, task_queue_name: str, block_ms=2000
    ) -> tuple[str, TaskEntry] | None:
        """
        ...
        try:
            ...
            for stream_name, messages in tasks:
                for message_id, data in messages:
                    ...
                    if task_id:
                        ...
                        if result_json:
                            ...
                            try:
                                ...
                                entry.start_at = datetime.now().strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                                await self.redis.set(
                                    result_key,
                    ...
                    log.error(
                        "Task found in stream but no valid entry in Redis",
                        task_id=task_id,
                    )
                    return
    ...
    async def update_task_result(
        ...
        try:
            ...
            if (
                ...
                or status == BrowserTaskStatus.FAILED
            ):
            ...
            self._notify_listeners(
                worker_id, task_id, status, entry.response.task_response
            )

            return True
        except Exception as e:
            log.exception(
                "Error updating task", error=str(e), exc_info=True, task_id=task_id
            )
            return False
