        ...
        task_response = await task_manager.submit_task(
            BrowserTaskRequest(
                task={
                    ...
                    "description": f"Open 'https://affiliate-us.tiktok.com/connection/creator?shop_region=US', wait for the page to finish rendering. Please do not scroll the page throughout. If there is a pop-up window or dialog box on the open page, click the close button in the upper right corner of the pop-up window to close it. The close button contains a symbol similar to the letter X, which is smallest in size. Before proceeding to the next step, make sure they are closed again and again. Then click on 'Find creators'. In the search box, type in: {data.uniqueId}, then press enter to search. In search result, find the row that has name exactly same as what you typed in search. Return the influencer name as name, GMV as gmv, Item Sold as item_sold, Avg video views as avg_video_views, and Engagement Rate as engagement_rate, in json format. 'final_result' is returned to me in JSON format, and it is not allowed to add any description",
                    "shop": {
                        "shop": "ShopperInc",
