        ...
        task_response = await task_manager.submit_task(
            BrowserTaskRequest(
                task={
                    ...
                    "description": f"Open 'https://affiliate-us.tiktok.com/connection/creator?shop_region=US', wait for the page to finish rendering. Must check for pop-ups on the page at every step of execution. If there is a pop-up window or dialog box on the open page, parsing the page code, find the 'svg' element with the class 'arco-icon-close' and click on it to close the pop-up window. Then click on 'Find creators'. In the search box, type in: {data.uniqueId}, then press enter to search. In search result, you need to click on the row that has name exactly same as what you typed in search and wait for the details page to finish loading. In the details page, resolve the cid from the URL. Return the influencer name as name, cid as cid, GMV as gmv, Item Sold as item_sold, Avg video views as avg_video_views, and Engagement Rate as engagement_rate, in json format. 'final_result' is returned to me in JSON format, and it is not allowed to add any description",
                    "shop": {
                        "shop": "ShopperInc",
